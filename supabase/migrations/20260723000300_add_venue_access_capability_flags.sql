-- Venue-level Open Gym / Private Rental capability flags for discovery filters.
-- This timestamp follows the deployed next-available corrective migration.
-- drop_in_price on venues is denormalized for card display; venue_admin_configs remains source of truth.

ALTER TABLE public.venues
  ADD COLUMN IF NOT EXISTS offers_open_gym BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS offers_private_rental BOOLEAN NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS drop_in_price NUMERIC NULL
    CHECK (drop_in_price IS NULL OR drop_in_price > 0);

COMMENT ON COLUMN public.venues.offers_open_gym IS
  'Discovery flag: venue offers drop-in open gym. Synced from venue_admin_configs.drop_in_enabled.';
COMMENT ON COLUMN public.venues.offers_private_rental IS
  'Discovery flag: venue offers private court rental (including request-to-book).';
COMMENT ON COLUMN public.venues.drop_in_price IS
  'Denormalized drop-in price for discovery cards. Source of truth: venue_admin_configs.drop_in_price.';

-- Backfill from admin config; keep offers_private_rental true for existing venues.
UPDATE public.venues v
SET
  offers_open_gym = COALESCE(vac.drop_in_enabled, false),
  drop_in_price = vac.drop_in_price,
  offers_private_rental = true
FROM public.venue_admin_configs vac
WHERE vac.venue_id = v.id;

UPDATE public.venues
SET offers_private_rental = true
WHERE offers_private_rental IS DISTINCT FROM true;

-- Extend combined regular/open-gym discovery with venue capability columns.
...
COMMENT ON FUNCTION public.get_venues_with_next_available IS
  'Returns active venues with capability flags and their earliest future regular rental or open-gym session, ordered chronologically.';
