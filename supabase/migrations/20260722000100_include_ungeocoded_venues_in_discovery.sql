-- Include active venues without a geocoded location in discovery.
-- /api/venues already returns these rows for access filters; search discovery
-- previously excluded them via get_venues_with_next_available requiring location.
-- Radius filtering still requires a location; ungeocoded venues sort last by distance.
...
COMMENT ON FUNCTION public.get_venues_with_next_available IS
  'Returns active venues (including ungeocoded) with next regular slot_instance plus Open Gym / Private Rental capability flags. Radius filters still require location.';
