-- Canonical regular-slot availability eligibility for parity across:
-- - /search and /venues next-available surfaces
-- - homepage featured availability
-- - venue availability API regular slot path

CREATE INDEX IF NOT EXISTS idx_bookings_active_conflict_lookup
  ON public.bookings (venue_id, date, start_time, end_time)
  WHERE status IN ('pending', 'confirmed');

CREATE INDEX IF NOT EXISTS idx_recurring_bookings_active_conflict_lookup
  ON public.recurring_bookings (venue_id, date, start_time, end_time)
  WHERE status IN ('pending', 'confirmed');

CREATE INDEX IF NOT EXISTS idx_slot_instances_regular_active_lookup
  ON public.slot_instances (venue_id, date, start_time, end_time)
  WHERE is_active = true
    AND action_type IN ('instant_book', 'request_private');
  ...
  base_regular_slots AS (
    ...
    WHERE si.is_active = true
      ...
      AND (si.date + si.start_time) >= (
        now_pt.ts + make_interval(hours => COALESCE(vac.min_advance_lead_time_hours, 0))
      )
      AND NOT (
...
COMMENT ON FUNCTION public.get_regular_available_slot_instances IS
  'Canonical regular slot eligibility (policy, blackout/holiday, external blocks, booking occupancy, and drop-in inventory blocking).';
...
COMMENT ON FUNCTION public.get_venues_with_next_available IS
  'Returns active venues with their next canonical regular available slot, filtered via shared eligibility rules.';
