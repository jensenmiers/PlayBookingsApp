-- Scope next-available discovery to the selected venue-access segment.
-- Hybrid venues remain members of both capability segments, while the
-- displayed next slot is selected from the matching slot type.
...
-- Preserve callers that predate access-scoped discovery.
CREATE OR REPLACE FUNCTION public.get_venues_with_next_available(
    p_date_filter DATE DEFAULT NULL,
    p_user_lat DOUBLE PRECISION DEFAULT NULL,
    p_user_lng DOUBLE PRECISION DEFAULT NULL,
...
AS $$
    SELECT *
    FROM public.get_venues_with_next_available(
      'all',
      p_date_filter,
      p_user_lat,
      p_user_lng,
...
GRANT EXECUTE ON FUNCTION public.get_venues_with_next_available(
  TEXT,
  DATE,
  DOUBLE PRECISION,
  DOUBLE PRECISION,
...
COMMENT ON FUNCTION public.get_venues_with_next_available(
  ...
  'Returns active venues in the selected capability segment with the earliest matching rental or open-gym slot.';
