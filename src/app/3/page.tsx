...
const FEATURED_COURT_LIMIT = 2

function VerticalCourtCard({
  court,
  index,
  mounted,
}: {
  ...
  return (
    ...
    >
      <div
        className={`relative overflow-hidden rounded-2xl bg-secondary-800/80 border border-secondary-50/10 transition-all duration-500 ${
          isHovered ? 'border-primary-400/40 shadow-xl shadow-primary-400/5' : ''
        }`}
      >
        {/* Vertical card: image on top with 4:3 aspect */}
        ...
        <div className="p-4 bg-secondary-800/80">
          <div className="flex items-baseline justify-between gap-2 mb-1">
            <h3 className="font-serif text-xl text-secondary-50 leading-tight truncate">
              {court.name}
            </h3>
        ...
        <div
          className={`absolute bottom-0 left-0 h-1 bg-gradient-to-r from-primary-400 to-accent-400 transition-all duration-500 ${
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-hidden">
      ...
      <section className="relative flex flex-col h-[calc(100dvh-4rem)] min-h-[calc(100dvh-4rem)] lg:flex-row">
        ...
        {/* OPTION 3: Vertical cards, 2 cards, more breathing room */}
        <div className="relative hidden lg:flex lg:w-[40%] lg:flex-col lg:justify-center lg:px-8 lg:py-8 xl:px-10">
            ...
            {featuredLoading ? (
              <>
                {[0, 1].map((i) => (
                  <div
                    key={i}
                    className="aspect-[4/3] rounded-2xl bg-secondary-800/50 border border-secondary-50/10 animate-pulse"
                  />
                ))}
              </>
            ) : featuredCourts.length > 0 ? (
              featuredCourts.map((court, i) => (
                <VerticalCourtCard key={court.id} court={court} index={i} mounted={mounted} />
              ))
            ) : (
