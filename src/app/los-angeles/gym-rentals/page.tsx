...
const ROUTE = findCityLanding('los-angeles', 'gym-rentals')!
...
export default async function LaGymRentalsPage() {
  const supabase = createPublicServerClient()
  const venues = await loadVenuesForCategory(supabase, 'gym-rentals')
  ...
  return (
    <LandingPageShell
      h1={ROUTE.h1}
      intro={ROUTE.intro}
      breadcrumbs={[{ href: '/', label: 'Home' }, { label: 'Los Angeles' }, { label: 'Gym Rentals' }]}
    >
      <JsonLd id="la-gym-rentals-jsonld" data={itemListJsonLd} />

      {groups.length === 0 ? (
        <p className="text-secondary-50/60">No active gym rentals yet — check back soon.</p>
      ) : (
          ...
          {groups.map((group) => (
            <section key={group.slug}>
              <div className="mb-4 flex items-end justify-between">
                <h2 className="text-2xl font-semibold">Gyms in {group.name}</h2>
                <Link
                  href={`${ROUTE.path}/${group.slug}`}
