...
async function main() {
  ...
  const { data, error } = await supabase.rpc('process_regular_template_sync_queue', {
    p_limit: Number.isInteger(limit) && limit > 0 ? limit : 25,
    p_horizon_days: Number.isInteger(horizonDays) && horizonDays > 0 ? horizonDays : 180,
  })

  if (error) {
    console.error(`Failed to process regular template sync queue: ${error.message}`)
    process.exit(1)
  }

  const rows = Array.isArray(data) ? data : []
  console.log(`Processed ${rows.length} venue(s) from regular template sync queue`)
